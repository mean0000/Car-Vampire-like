...
namespace CharacterCustomizationTool.Editor.Character
{
    public class FullBodySlot : SlotBase
    {
        ...
        public override (SlotType, Mesh, Material[])[] Meshes => _selected.Elements.Select(e => (e.Type, e.Mesh, e.Materials)).ToArray();

        public FullBodySlot(FullBodyEntry[] fullBodyEntries) : base(SlotType.Costume)
        {
        ...
        {
            if (groupType == GroupType.Costume)
            {
        ...
        {
            if (!isEnabled || groupType != GroupType.Costume)
            {
                return false;
            }
        ...
        protected override void DrawSlot(int previewLayer, Camera camera)
        {
            ...
            {
                DrawMesh(element.Mesh, element.Materials, previewLayer, camera);
            }
        }
    }
}