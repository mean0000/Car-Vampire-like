...
namespace CharacterCustomizationTool.Editor
{
    public class CharacterCustomizationWindow : EditorWindow
    {
        ...
        private static void Init()
        {
            ...
            window.minSize = new Vector2(1310, 655);
            window.Show();
        }
        ...
        private void OnGUI()
        {
            ...
            GUI.Label(new Rect(10, 372, 40, 20), "Name:");
            _prefabName = GUI.TextField(new Rect(55, 372, 255, 20), _prefabName);
        ...
        private void SavePrefab()
        {
            ...
            foreach (var slot in enabledSlots)
            {
                ...
                {
                    var t = character.transform.Cast<Transform>().FirstOrDefault(t =>
                    {
                        var n = meshInfo.Item1.ToString();
                        if (n.EndsWith("s"))
                        {
                            n = n[..^1];
                        }

                        return string.Join("", t.name.Split('_').ToArray()).StartsWith(n);
                    });
                    if (t == null)
                    {
                        Debug.Log($"Not found: {meshInfo.Item1}");
                        continue;
                    }
            ...
            Directory.CreateDirectory(AssetsPath.SavedCharacters);
            _prefabName = string.IsNullOrEmpty(_prefabName) ? defaultCharacterName : _prefabName;
            var path = AssetDatabase.GenerateUniqueAssetPath($"{AssetsPath.SavedCharacters}{_prefabName}.prefab");
