...
## ★변수2 = 전진 스텝인 (Animation 설계 완료, 미구현 = phase2)
...
**경로 비교:**
- **경로2 (권고)**: `S1_Combo01_01_Retimed.anim`의 **RootT.z에 전진 ramp 저작** +0.35~0.45m, **hit(norm0.28)까지 싣고 회수 flat hold = DD 정석("밀어넣고 멈춤")**. 수평베기·이벤트·리타이밍·컨트롤러 전부 보존. 유일하게 "기본 베기 + 깨끗한 스텝인".
...
2. **변수2 전진 스텝인 구현** — 경로2 권고(+0.4m), 백업 먼저 + retimed `.anim` 직접 편집. Animation phase2(SendMessage로 지시).
