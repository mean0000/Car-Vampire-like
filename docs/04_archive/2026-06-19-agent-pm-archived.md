...
> 🟥 **ARCHIVED 2026-06-19 — 프로젝트 활성 로스터에서 제외(병합).** 적대적/데블스애드보킷 기능 → 신규 `Challenger`(게임-인지: 레퍼·캐넌·메모리 무장)로 흡수. 시스템 이데이션 → `GameDesign`. 일반 PM(B2C/B2B·10x/100x 스케일·비즈 스테이크홀더) 프레이밍은 게임 설계에 부적합. 복구 시: 글로벌 동명(`~/.claude/agents/pm.md`) 충돌 확인. 사유·권위 = memory [[project_2026_06_19_competition_governance]].
