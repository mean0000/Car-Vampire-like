...
> 🟥 **ARCHIVED 2026-06-19 — 프로젝트 활성 로스터에서 제외.** 오프라인 솔로 Unity 게임엔 DB/서버/API/스케일링 무관, 사용 기록 0건(docs·memory 언급 0, agent-memory 폴더 유일하게 없음). 복구 시: 이 파일을 `.claude/agents/`로 되돌리고 글로벌 동명(`~/.claude/agents/backend.md`)과의 충돌 확인. 사유·권위 = memory [[project_2026_06_19_competition_governance]].
